...
The prompt also includes a dedicated block when `runGate === "spec_export_invalid"` that instructs Devin to fix the spec source first (e.g. add requestBody to JSDoc), re-run the export, then update the published spec and docs ([see Run Gate Logic](#6.4)).

This conditional block:
- Activates only when `runGate === "infer"`
