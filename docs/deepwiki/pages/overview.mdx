...
graph TB
    ...
    subgraph RunGate["Run Gate Decision"]
        ...
        SpecDrift["spec_drift<br/>(highest confidence)"]
...
The detection system produces one of four run gates that determine whether remediation proceeds:

- `spec_drift`: API specification changes detected (highest priority)
