...
| `baseline_drift` | Current export differs from published spec at `lastKnownBaseline` commit | Full remediation (API changed since last sync) | Any mode |
| `baseline_missing` | `lastKnownBaseline` blank and no other drift | Assume drift; proceed (first install / cold start) | Any mode |
