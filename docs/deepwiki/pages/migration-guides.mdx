...
The system implements three distinct PR creation patterns based on trigger type:
- **Push to main**: Creates standard PRs with normal branch naming
- **Pull request (new)**: Creates draft PRs with `docdrift/pr-{N}` branch naming and posts a comment linking them to the source PR
- **Pull request (existing)**: Finds and updates the existing doc-drift PR via `findExistingDocdriftPrForSource`, avoiding PR proliferation
...
| `GITHUB_PR_NUMBER` | Workflow extraction | Source PR number for draft PR creation | No |
