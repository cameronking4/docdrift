...
This document explains how docdrift manages the lifecycle of documentation updates created by Devin AI. When the trigger is **pull request**, the default behavior is **commit-to-branch**: Devin commits directly to the source PR branch (no separate docdrift PR). Alternatively, with `devin.prStrategy: "separate-pr"`, docdrift creates or updates a `docdrift/pr-N` draft PR. For **push** to main or **manual** runs, the system creates or updates a single docdrift branch PR. This page covers branch naming, comment posting, issue creation, and state tracking.
...
## PR and commit patterns

- **Pull request + commit-to-branch** (default, `devin.prStrategy: "commit-to-branch"`): Devin commits to the source PR branch. No separate docdrift PR; no `postPrComment`. The workflow must pass `GITHUB_PR_HEAD_REF` and `--pr-head-ref`.
- **Pull request + separate-pr** (`devin.prStrategy: "separate-pr"`): Same as the legacy per-PR flow — create or update a `docdrift/pr-N` draft PR; optionally post a comment on the source PR when a new docdrift PR is opened.
- **Push to main / manual**: Create or update a single docdrift branch PR (or per-docdrift PR when `branchStrategy: "per-pr"` with separate-pr).
...
flowchart TD
    ...
    Trigger -->|pull_request| PRStrategy{prStrategy?}
    
    PRStrategy -->|commit-to-branch| CommitPath["Commit-to-Branch Pattern"]
    PRStrategy -->|separate-pr| CheckExisting["Check for existing<br/>docdrift PR"]
    
    CheckExisting --> Found{PR Found?}
    Found -->|Yes| UpdatePath["Update existing docdrift/pr-N"]
    Found -->|No| DraftPath["Create DRAFT PR<br/>docdrift/pr-N"]
    
    PushPath --> StandardPR["Create/update PR<br/>Branch: docdrift or default"]
    
    CommitPath --> CommitBranch["Devin commits to source PR branch<br/>No new PR"]
    
    DraftPath --> PostPRComment["postPrComment on source PR"]
    UpdatePath --> PostCommit2["postCommitComment"]
    StandardPR --> PostCommit["postCommitComment"]
    CommitBranch --> PostCommitC["postCommitComment only"]
    
    style PRStrategy fill:#f9f9f9
    style Trigger fill:#f9f9f9
```

**Diagram 1: PR / commit pattern decision tree**

Sources: [src/index.ts](), [src/devin/prompts.ts](), [src/github/client.ts]()
...
### Commit-to-branch pattern (pull request, default)

When triggered by a `pull_request` event and `devin.prStrategy` is `commit-to-branch` (default), the system does **not** create a separate docdrift PR. Devin is instructed to checkout the source PR branch (via `prHeadRef` / `GITHUB_PR_HEAD_REF`), apply doc changes, commit, and push. Doc updates appear as commits on the same PR. No `postPrComment` is posted. This is the lowest-noise option for PR-driven runs.

### Draft PR pattern (pull request, separate-pr)

When `devin.prStrategy` is `separate-pr` and triggered by a `pull_request` event with no existing docdrift PR, the system creates a **draft PR** with special branch naming:
