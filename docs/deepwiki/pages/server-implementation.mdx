...
**Syntax:**
```bash
docdrift run [--base <sha>] [--head <sha>] [--trigger <type>] [--pr-number <n>]
```
...
- `GITHUB_PR_NUMBER` — Used to auto-resolve PR number
...
# Run for pull request trigger
docdrift run --trigger pull_request --pr-number 42
