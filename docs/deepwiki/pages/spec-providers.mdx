...
specProviders:
  - format: openapi3              # Required: Specification format
    ...
    validation:                   # Optional: OpenAPI export completeness validation
      enabled: true
      allowlist: ["/health"]
```
...
The `specProviderConfigSchema` defines three required fields and an optional `validation` block:
...
| `validation` | object | Optional. OpenAPI export completeness validation (see below). |

**Validation object (optional):**

| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `enabled` | boolean | `true` | Enable export completeness validation for OpenAPI specs. |
| `allowlist` | string[] | `[]` | Path patterns to skip during validation (e.g. `/health`). |
...
For OpenAPI spec providers using `current.type: export`, the exported specification is validated for completeness before it is compared to the published spec. This ensures the export is the source of truth and avoids undetected drift when both generated and published specs are missing the same fields (e.g. incomplete `@swagger` JSDoc omitting `requestBody`).

**Validation rules:**

- Every path operation with HTTP method POST, PUT, or PATCH must have a `requestBody`.
- If `requestBody` is present, its schema must have `properties` (non-empty request body schema).

Paths listed in `validation.allowlist` are skipped (e.g. `/health` for a health-check endpoint with no body).

When validation fails, the OpenAPI detector:

1. Writes validation issues to `.docdrift/openapi3-validation.txt`
2. Returns `hasDrift: true` with a signal of kind `spec_export_incomplete` (tier 1)
3. Causes the run gate to be set to `spec_export_invalid`

Devin is then prompted to fix the spec source first (e.g. add `requestBody` to JSDoc or schema), re-run the export command, then update the published spec and documentation. Validation is controlled per provider via `validation.enabled` (default `true`) and `validation.allowlist`.

**Sources:** [src/spec-providers/openapi-validator.ts](), [src/spec-providers/openapi.ts](), [src/detect/index.ts:70-76]()
...
graph TB
    ...
    RunGate -->|"anySpecExportInvalid"| SpecExportInvalid
...
- Items: `object` with properties:
  ...
  - `validation`: optional object with `enabled` (boolean, default true) and `allowlist` (string[])
