...
- **Triggers:** Push to `main`, **pull request** (to `main`), or `workflow_dispatch`. On pull request, docdrift runs on the **PR branch** (base/head SHAs from the PR). The job is skipped when the PR head branch starts with `docdrift/` to avoid running on docdrift’s own branches.
- Action uploads: `.docdrift/drift_report.json`, `.docdrift/evidence/**`, `.docdrift/metrics.json`
...
- **Pull request** — runs when a PR targets `main`. Docdrift evaluates the **PR branch** (not main). With default `devin.prStrategy: "commit-to-branch"`, Devin commits doc fixes to the same PR branch; no separate docdrift PR.
